...
boolean FtpServer::processCommand()
{
  ...
  else if( ! strcmp( command, "LIST" ))
  {
     ...
     if((!dir.isDirectory())){
       Serial.println( "!dir.isDirectory() " + String(dir.isDirectory()) );
     }
     if((!dir)||(!dir.isDirectory())){
        Serial.println( "*550 Can't open directory " + String(cwdName) );
...
//      Dir dir=SD.openDir(cwdName);
      ...
      if( !SD_MMC.exists( cwdName )){
        client.println( "550 Can't open directory " + String(parameters));
        Serial.println( "***550 Can't open directory " + String(parameters));
      }else{
          File file = dir.openNextFile();
//        while( dir.next())
        while( file)
