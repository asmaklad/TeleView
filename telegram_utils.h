...
String formulateOptionsInlineKeyBoard(){
  ...
  keyboardJson += R"([{ "text" : "start an FTP server for the SD. )";
  keyboardJson += (configItems.ftpServerOn?"ON\u2705":"OFF\u274C");
  keyboardJson += R"(", "callback_data" : "/ftpServerOn" }],)";

  keyboardJson += R"([{ "text" : "Save Video to SD. )";
  keyboardJson += (configItems.videoToSDOn?"ON\u2705":"OFF\u274C");
  keyboardJson += R"(", "callback_data" : "/videoToSDOn" }],)";

  keyboardJson += R"([{ "text" : "Send Video on/off. )";
  keyboardJson += (configItems.sendVideoOn?"ON\u2705":"OFF\u274C");
  keyboardJson += R"(", "callback_data" : "/sendVideoOn" }],)";
    ...
    if (chat_id.equals(configItems.adminChatIds) || chat_id.equals(configItems.userChatIds)) {
      ...
      if (bot.messages[i].type ==  F("callback_query")) {
        ...
        if ( bInlineKeyboardExtraOptions ){
          ...
          }else if (text == "/ftpServerOn") {
            configItems.ftpServerOn = !configItems.ftpServerOn;
          }else if (text == "/videoToSDOn") {
            configItems.videoToSDOn = !configItems.videoToSDOn;
          }else if (text == "/sendVideoOn") {
            configItems.sendVideoOn = !configItems.sendVideoOn;
